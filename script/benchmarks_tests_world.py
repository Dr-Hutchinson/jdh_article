...
us_sheet = gc.open('duplicate_high_school_world_history_test')
...
with col2:
    ...
    def world_history_data():
        ...
        st.write('ChatGPT has correctly answered {} out of {} World History questions, for a {:.2f}% accuracy rate.'.format(len(correct_data), len(field_data), len(correct_data)/len(field_data)*100))
        st.write("Below is ChatGPT total accuracy rate to date.")
        st.bar_chart(data['correct_status'].value_counts())

    def world_history_data_gpt4():
